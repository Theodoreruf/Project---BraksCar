...
def tracker(color_frame, mask, color_infos):
    ...
    if(len(elements)>0):
        ...
        if(w*h>10):
            ...
            cv.circle(color_frame, (int(w/2),int(h/2)), 5 ,color_infos,10) #point centrzl de l'objet
            cv.line(color_frame,(int(x),int(y)),(int(x)+150,int(y)),color_infos,2)
            cv.putText(color_frame, "object" ,(int(x)+10, int(y) -50), cv.FONT_HERSHEY_DUPLEX, 1, color_infos, 1, cv.LINE_AA)
